...
//--------------------INSERT VENDOR CATEGORIES----------------
    ...
    //Convert vendor_name to viable vendor_url
    urlSafe: function (vendorName) {
        var str = vendorName.trim();
        var str2 = str.replace(/\s+/g, '-').toLowerCase();
        var str3 = str2.replace(/\(|\)|!|\'|\./g, '');
        return str3.replace(/&/g, 'amp');
    },

    //Get rid of http:// and https:// for adding and editing vendors
    stripHttp: function (vendorWebsite) {
        var str = vendorWebsite;
        var str2 = str.replace(/^(https?|ftp):\/\//, '');
        return str2;
    }
};